...
contains
  ...
  subroutine send_info_to_neighbours(this, field)
    ...
    if (this%myright /= MPI_PROC_NULL) then
    do j = 1, this%ny
    ...
    do j = 1, this%ny
      field(this%nx + 1, j)%T = field(this%nx, j)%T
      field(this%nx + 1, j)%sT = field(this%nx + 1, j)%T

      field(this%nx + 1, j)%F = field(this%nx, j)%F
      field(this%nx + 1, j)%sF = field(this%nx + 1, j)%F

      field(this%nx + 1, j)%Z = field(this%nx, j)%Z
      field(this%nx + 1, j)%sZ = field(this%nx + 1, j)%Z
    end do
    end if
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (this%myleft /= MPI_PROC_NULL) then
    do j = 1, this%ny
      field(0, j)%T = mrl(j)
    ...
    if (this%mybot /= MPI_PROC_NULL) then
    do j = 1, this%nx
    ...
    do i = 1, this%nx
      field(i, 0)%T = field(i, 1)%T
      field(i, 0)%sT = field(i, 1)%T

      field(i, 0)%F = field(i, 1)%F
      field(i, 0)%sF = field(i, 1)%F

      field(i, 0)%Z = field(i, 1)%Z
      field(i, 0)%sZ = field(i, 1)%Z
    end do
    end if
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !! Sending info top
    ...
    if (this%mytop /= MPI_PROC_NULL) then
    do j = 1, this%nx
      field(j, 0)%T = mrb(j)
    ...
    do i = 1, this%nx
      field(i, this%ny + 1)%T = field(i, this%ny)%T
      field(i, this%ny + 1)%sT = field(i, this%ny)%T

      field(i, this%ny + 1)%F = field(i, this%ny)%F
      field(i, this%ny + 1)%sF = field(i, this%ny)%F

      field(i, this%ny + 1)%Z = field(i, this%ny)%Z
      field(i, this%ny + 1)%sZ = field(i, this%ny)%Z
    end do
    end if
  ...
  subroutine test(this)
    ...
    print "(A16, I2, A18, I2, A11)", "My processor is:", this%myproc, " from the pool of ", &
