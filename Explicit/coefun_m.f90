...
contains
  ...
  subroutine explicitstep(w_d, e_d, s_d, n_d, o_d, ind, step0, sol)
    real(DP), intent(in) :: w_d(:, :), e_d(:, :), o_d(:, :), &
                            s_d(:, :), n_d(:, :), ind(:, :), step0(:, :)
    ...
    do concurrent(i=2:nx - 1)
      do concurrent(j=2:ny - 1)
        sol(i, j) = ind(i, j) - o_d(i, j)*step0(i, j)
        if (i > 1) sol(i, j) = sol(i, j) - w_d(i, j)*step0(i - 1, j)
        if (i < nx) sol(i, j) = sol(i, j) - e_d(i, j)*step0(i + 1, j)
        if (j > 1) sol(i, j) = sol(i, j) - s_d(i, j)*step0(i, j - 1)
        if (j < ny) sol(i, j) = sol(i, j) - n_d(i, j)*step0(i, j + 1)

      end do
    end do

  end subroutine explicitstep
