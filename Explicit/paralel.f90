program main
  ...
  use coefun_m, only: ADI, write_tstep, gauss_seidel, explicitstep
  ...
  do t = 1, param%nt
    ...
    call explicitstep(field(0:PL%nx + 1, 0:PL%ny + 1)%aT(1), field(0:PL%nx + 1, 0:PL%ny + 1)%aT(2), &
                      field(0:PL%nx + 1, 0:PL%ny + 1)%aT(3), field(0:PL%nx + 1, 0:PL%ny + 1)%aT(4), &
                      field(0:PL%nx + 1, 0:PL%ny + 1)%aT(5), field(:, :)%sT, field0(:, :)%T, field(:, :)%T)

    field(:, :)%T = field(:, :)%T/param%txy
    tmperror = norm2(field(:, :)%T - field0(:, :)%T)
    error = max(tmperror, 0.0_DP)

    call explicitstep(field(0:PL%nx + 1, 0:PL%ny + 1)%aF(1), field(0:PL%nx + 1, 0:PL%ny + 1)%aF(2), &
                      field(0:PL%nx + 1, 0:PL%ny + 1)%aF(3), field(0:PL%nx + 1, 0:PL%ny + 1)%aF(4), &
                      field(0:PL%nx + 1, 0:PL%ny + 1)%aF(5), field(:, :)%sF, field0(:, :)%F, field(:, :)%F)

    field(:, :)%F = field(:, :)%F/param%txy
    error = max(tmperror, error)
    tmperror = norm2(field(:, :)%F - field0(:, :)%F)

    call explicitstep(field(0:PL%nx + 1, 0:PL%ny + 1)%aZ(1), field(0:PL%nx + 1, 0:PL%ny + 1)%aZ(2), &
                      field(0:PL%nx + 1, 0:PL%ny + 1)%aZ(3), field(0:PL%nx + 1, 0:PL%ny + 1)%aZ(4), &
                      field(0:PL%nx + 1, 0:PL%ny + 1)%aZ(5), field(:, :)%sZ, field0(:, :)%Z, field(:, :)%Z)

    field(:, :)%Z = field(:, :)%Z/param%txy
    tmperror = norm2(field(:, :)%Z - field0(:, :)%Z)
    error = max(tmperror, error)
